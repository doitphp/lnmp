...
printf "\n"
printf "========================================\n"
printf " Nginx V1.7.10 + jemalloc V3.6.0 Install \n"
...
if [ -s nginx-1.7.10.tar.gz ]; then
    echo "nginx-1.7.10.tar.gz [found]"
else
    echo "nginx-1.7.10.tar.gz download now..."
    wget http://nginx.org/download/nginx-1.7.10.tar.gz
fi

nginxMd5=`md5sum nginx-1.7.10.tar.gz | awk '{print $1}'`
if [ "$nginxMd5" != "8e800ea5247630b0fcf31ba35cb3245d" ]; then
    echo "Error: nginx-1.7.10.tar.gz package md5 value is invalid. Please check package download url";
    exit 1
fi
